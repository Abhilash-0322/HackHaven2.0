...
function ChatPage({ user }) {
  ...
  const selectThread = async (thread) => {
    setActiveThread(thread); setMobileThreads(false)
    try {
      const data = await apiFetch(`/mental-health/threads/${thread.id}`)
      setMessages((data.messages || []).map((message) => ({ ...message, id: message.id || `${message.thread_id}-${message.timestamp}` })))
    } catch { /* keep the current room if the API is unavailable */ }
  }
  const sendMessage = async (event) => {
    ...
    setMessages((current) => [...current, { id: Date.now(), content: message, is_user: true }, { id: replyId, content: '', is_user: false, streaming: true }])
