...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    setLoading(true);
    try { await register(form); navigate('/chat', { replace: true }); }
  ...
  return (
    <div className="min-h-screen bg-cream flex">
      <div className="hidden lg:flex lg:w-1/2 bg-editorial-ink text-cream p-12 flex-col justify-center">
        ...
        <QuoteBlock quote="You yourself, as much as anybody in the entire universe, deserve your love and affection." attribution="Buddha" className="border-terracotta text-cream/80 !text-xl" />
      </div>
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <Input label="Name" type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Your name" />
          ...
          <p className="mt-6 text-center font-sans text-sm text-charcoal-muted">Already have an account? <Link to="/login" className="editorial-link">Sign in</Link></p>
        </div>
      </div>
    </div>
  );
