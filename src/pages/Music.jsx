...
export default function Music() {
  ...
  const getRecommendations = async (song) => {
    ...
    try {
      const data = await musicApi.getRecommendations(song);
  ...
  const openSpotify = (uri) => {
    if (uri) window.open(uri.replace('spotify:track:', 'https://open.spotify.com/track/'), '_blank');
  };
    ...
    <div className="editorial-container py-8 md:py-12">
      ...
      <div className="relative max-w-xl mb-10">
        <div className="flex items-center border border-editorial-border rounded-sm bg-cream-light px-4 py-3">
          ...
          <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search for a song..."
            className="flex-1 bg-transparent outline-none font-sans text-charcoal placeholder:text-charcoal-muted" />
        ...
        {showDropdown && (
          <ul className="absolute z-10 w-full mt-1 border border-editorial-border bg-cream-light rounded-sm shadow-editorial max-h-60 overflow-y-auto">
            {filtered.map((song) => (
              <li key={song}>
                <button type="button" onClick={() => getRecommendations(song)} className="w-full text-left px-4 py-2.5 font-sans text-sm hover:bg-cream-dark transition-colors">{song}</button>
              </li>
            ))}
          </ul>
        )}
      ...
      {initialLoading ? <div className="flex justify-center py-16"><Spinner size="lg" label="Loading music library..." /></div>
        ...
        : recommendations.length === 0 ? (
          <EmptyState icon={Music2} title="Search for a song" description="Enter a song name above to discover similar tracks for your wellness playlist." />
        ) : (
          <div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {recommendations.map((song, i) => (
                <Card key={i}>
                  ...
                  {song.spotify_uri && (
                    <button type="button" onClick={() => openSpotify(song.spotify_uri)} className="mt-4 flex items-center gap-2 text-sm font-sans text-terracotta hover:text-terracotta-dark">
                      <Play className="w-4 h-4" /> Open in Spotify
                    </button>
                  )}
                </Card>
              ))}
