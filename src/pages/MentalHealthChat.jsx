...
import { MessageCircle, Send, ChevronRight, HelpCircle, Book, X, Mic, MicOff, Volume2, VolumeX } from 'lucide-react';
...
const MentalHealthChat = () => {
  ...
  const speakText = (text) => {
    ...
    // Choose a soft, friendly voice if available
  ...
  const sendMessage = async (messageText) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/mental-health/chat`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-indigo-50 to-white">
          ...
          <div className="bg-white rounded-xl shadow-md p-3 border border-gray-200">
            ...
            <div className="mt-2 border-t border-gray-100 pt-2 px-2">
              <p className="text-xs text-gray-500">
                {isListening ? (
                  <span className="text-indigo-600 font-medium">Listening... Say something</span>
                ) : (
      ...
      {isListening && (
        <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 bg-indigo-600 text-white px-4 py-2 rounded-full shadow-lg flex items-center space-x-2">
