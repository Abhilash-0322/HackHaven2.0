...
export const chatApi = {
  getThreads: (limit = 20, offset = 0) =>
    apiRequest(`/mental-health/threads?limit=${limit}&offset=${offset}`),
  getThread: (threadId) => apiRequest(`/mental-health/threads/${threadId}`),
  deleteThread: (threadId) =>
    apiRequest(`/mental-health/threads/${threadId}`, { method: 'DELETE' }),
  updateThreadTitle: (threadId, title) =>
    apiRequest(`/mental-health/threads/${threadId}/title`, {
      method: 'PUT',
...
export const journalApi = {
  ...
  getEntry: (id) => apiRequest(`/journal/entries/${id}`),
  createEntry: (entry) =>
    apiRequest('/journal/entries', { method: 'POST', body: JSON.stringify(entry) }),
  updateEntry: (id, entry) =>
    apiRequest(`/journal/entries/${id}`, { method: 'PUT', body: JSON.stringify(entry) }),
  deleteEntry: (id) => apiRequest(`/journal/entries/${id}`, { method: 'DELETE' }),
  analyzeMood: (content) =>
    apiRequest('/journal/analyze-mood', { method: 'POST', body: JSON.stringify({ content }) }),
  getPrompts: () => apiRequest('/journal/prompts'),
  getInsights: () => apiRequest('/journal/insights'),
};

export const booksApi = {
  recommendByMood: (userId) =>
    apiRequest(`/books/recommend-by-mood${userId ? `?user_id=${userId}` : ''}`),
  ...
  recommendById: (bookId) => apiRequest(`/books/recommend/${encodeURIComponent(bookId)}`),
};

export const musicApi = {
  getSongs: () => apiRequest('/songs'),
  recommend: (song) => apiRequest(`/recommend?song=${encodeURIComponent(song)}`),
  getRecommendations: (song) => apiRequest(`/recommend?song=${encodeURIComponent(song)}`),
};
...
export const therapistsApi = {
  ...
  getSpecializations: () => apiRequest('/therapists/specializations'),
  bookAppointment: (appointment) =>
    apiRequest('/therapists/appointments', {
      method: 'POST',
      body: JSON.stringify(appointment),
    }),
  getUserAppointments: (userId) => apiRequest(`/therapists/appointments/user/${userId}`),
  cancelAppointment: (appointmentId) =>
    apiRequest(`/therapists/appointments/${appointmentId}`, { method: 'DELETE' }),
};
...
export const coinsApi = {
  ...
  getExchangeRates: () => apiRequest('/coins/exchange-rates'),
  spend: (amount, source, description) =>
    apiRequest('/coins/spend', {
      method: 'POST',
      body: JSON.stringify({ amount, source, description }),
    }),
  redeem: (amount, source, description) =>
    apiRequest('/coins/spend', {
      method: 'POST',
      body: JSON.stringify({ amount, source, description }),
    }),
  earn: (amount, source, description) =>
    apiRequest('/coins/earn', {
      method: 'POST',
      body: JSON.stringify({ amount, source, description }),
    }),
};
